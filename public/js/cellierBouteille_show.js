document.addEventListener("DOMContentLoaded", function () {
    ...
    const wrapper = document.querySelector(
        ".form-modifier-item > .select-wrapper"
    );
    const ajouterMillesime = document.querySelector(
        '[name="ajouterMillesime"]'
    );
    ...
    selectBouteille.addEventListener("change", (e) => {
        location.href =
            location.origin +
            `/cellier/${selectCellier.value}/${e.target.value}`;
    });
    ...
    let elMillesime =
        document.querySelector("[data-millesime]").dataset.millesime;
    ...
    M.Datepicker.init(datepicker, { autoClose: true, format: "yyyy-mm-dd" });
        ...
        note.addEventListener("change", (e) => {
            if (wrapper.classList.contains("hide")) {
                const millesime = document.querySelector(
                    ".millesime-item-selected"
                ).parentElement.dataset.jsBouton;
    ...
    for (let i = 0; i < boutonMillesime.length; i++) {
        boutonMillesime[i].addEventListener("click", function (e) {
            ...
            boutonMillesime[i]
                ...
                .classList.add("millesime-item-selected");
            /* Fetch cellier_Bouteille */

            fetch(
                `/obtenirMillesime/${idCellier}/${idBouteille}/${elMillesime}`
            )
                .then((response) => {
                    return response.json();
                })
                .then((response) => {
                    if ((value = response[0].millesime != 0)) {
                        infoForm.querySelector("#millesime").value =
                            response[0].millesime;
                    } else {
                        infoForm.querySelector("#millesime").value = "N/A";
                    }
                    infoForm.querySelector("#quantite").value =
                        response[0].quantite;
                    ...
                    infoForm
                        ...
                        .forEach((etoile) => {
                            if (etoile.dataset.value < response[0].note) {
                                ...
                                etoile.dataset.value == response[0].note
                            ) {
                                etoile.classList.add(
                                    "gl-active",
                                    "gl-selected"
                                );
                            } else {
                                etoile.classList.remove(
                                    "gl-active",
                                    "gl-selected"
                                );
                            }
                        });
                    infoForm.querySelector("#prix").value = parseFloat(
                        response[0].prix
                    ).toFixed(2);

                    infoForm.querySelector("#commentaire").value =
                        response[0].commentaire;
                    infoForm.querySelector("#date_achat").value =
                        response[0].date_achat;
                    infoForm.querySelector("#garde_jusqua").value =
                        response[0].garde_jusqua;
                })
                .catch((error) => console.log(error));
        });
        ...
        if (localStorage.getItem("millesime") != null) {
            if (
                localStorage.getItem("millesime") ==
                boutonMillesime[i].dataset.jsBouton
            ) {
    ...
    /** Bouton valider **/
    const btnValideActiveModal = document.querySelector(
        "[data-js-validerModal]"
    );
    btnValideActiveModal.addEventListener("click", function (e) {
        ...
        fetch(
            `/modifierCellierBouteille/${idCellier}/${idBouteille}/${annee}/${prix.value
                .trim()
                .replaceAll(".", "~point~")}/${quantite.value}/${
                date_achat.value
            }/${commentaire.value}/${garde_jusqua.value}`
        )
            .then((response) => {
                return response.json();
            })
            .then((response) => {
                if (response.errors) {
                    ...
                    if (estValide()) {
                        ...
                        infoForm.querySelector("#quantite").value = parseInt(
                            infoForm.querySelector("#quantite").value,
                            10
                        );
                        infoForm.querySelector("#prix").value = parseFloat(
            ...
            .catch((error) =>
                console.log("Le fetch ne fonctionne toujours pas", error)
            );
    });

    /* Bouton supprimer */

    const btnEffacerActiveModal = document.querySelector(
        "[data-js-suprimerModal]"
    );
    ...
    btnEffacerActiveModal.addEventListener("click", function (e) {
        ...
        fetch(
            `/suprimerCellierBouteille/${idCellier}/${idBouteille}/${elMillesime}`
        )
            .then((response) => {
                return response.json();
            })
    ...
    /**
     * Afficher le formulaire pour ajouter une bouteille dans la table cellier_bouteilles
     */
    ajouterMillesime.addEventListener("click", () => {
        ...
        infoForm.querySelector("#commentaire").value = "";
        infoForm.querySelector("#date_achat").value = new Date()
            .toISOString()
            .slice(0, 10);
    ...
    btnAjouterMillesime.addEventListener("click", (e) => {
        ...
        fetch(
            `/ajouterCellierBouteille/${idCellier}/${idBouteille}/${
                selectMillesime.value
            }/${note}/${prix.value.trim().replaceAll(".", "~point~")}/${
                quantite.value
            }/${date_achat.value}/${commentaire.value}/${garde_jusqua.value}`
        )
            .then((response) => {
                return response.json();
            })
    ...
    /**
     * Vérifier si le millésime selectionné existe déjà  
     */
    selectMillesime.addEventListener("change", () => {
        ...
        wrapper
            .querySelector(".select-dropdown")
            .classList.remove("champNonValide");
        btnAjouterMillesime.classList.remove("boutonNonValide");
        btnAjouterMillesime.removeAttribute("disabled");
        boutonMillesime.forEach((bouton) => {
